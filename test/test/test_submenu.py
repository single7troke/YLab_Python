...
@pytest.mark.asyncio
async def test_no_any_submenu_in_menu(create_menu, clear_db):
    await clear_db
    global MENU
...
@pytest.mark.asyncio
async def test_get_all_submenu():
...
async def test_update_submenu():
    global SUBMENU
    resp = await update_request(url=config.sub_menu_url, menu_id=MENU.id, submenu_id=SUBMENU.id, data=submenu_update)
    assert resp.status_code == 200
    assert check_header(resp.headers)
...
@pytest.mark.asyncio
async def test_submenu_have_been_deleted(clear_db):
    resp = await get_request(url=config.sub_menu_url, menu_id=MENU.id, submenu_id=SUBMENU.id)
